...
/**
 * This service is responsible for loading a careplan, and a target for binding to the careplan
 */
rhizomeBase.factory('rhsCareplan', function ($rootScope, jsonpService) {
    var svc = {}

    svc.loadCareplan = function (url) {
    ...
    // this is a hack, for now. should just call service
    $rootScope.$on('RhzCareplanUrl', function (evt, obj) {
        svc.loadCareplan(obj.url + '.json')
    });
